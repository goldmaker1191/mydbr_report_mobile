...
args.unshift(event,delta,deltaX,deltaY);return($.event.dispatch||$.event.handle).apply(this,args)}})(jQuery);jQuery(document).ready(function($){$('head').append('<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">');$("body").append('<div id="top_filters"><div class="handler"><span class="opener"><i class="arrow down"></i></span><span class="closer"><i class="arrow up"></i></span></div></div>');var topFilters=$("body").find("#top_filters");$(topFilters).on('click','.handler',function(event){event.preventDefault();$(topFilters).toggleClass('open');$('#dbr_rt1').toggleClass('filter_open');$(".divTableWithFloatingHeader .divTableWithFloatingHeader").customScrollbar("scrollToY",0);$("body").find("#thead_clone").toggle()});$("body").find(".tableFloatingHeaderOriginal:eq(1) .row_header th").each(function(index,el){$(topFilters).append('<button data-col="'+(index+1)+'" class="col_hide">'+$(this).text()+'</button>')});$(topFilters).css("top","-"+$(topFilters).height()-25+"px");$("body").append("<style> .filter_open { margin-top: "+($(topFilters).height()+60)+"px !important;} </style>");$("body").on('click','.col_hide',function(event){event.preventDefault();$(this).toggleClass('inactive');$('#dbr_rt1 tr > *:nth-child('+$(this).data('col')+')').toggle();$('.inner > *:nth-child('+$(this).data('col')+')',thead_clone).toggle();refreshScrollbar()});$('#dbr_rt1 tbody td').addClass('inline-edit');$("body").append('<div id="thead_clone"><div class="inner"></div></div>');var theadClone=$("body").find("#thead_clone");$("body").find(".tableFloatingHeaderOriginal:eq(1) .row_header th").each(function(index,el){$(".inner",theadClone).append('<div data-col="'+(index+1)+'" class="" style="width:'+$(this).width()+'px">'+$(this).text()+'</div>')});$("body").append('<div class="bt-scroller"><span id="rightScroller">&#x25B6;</span><span id="leftScroller">&#x25C0;</span><center><span id="topScroller">&#x25BC;</span></center></div><div id="custom_css"></div>');function refreshSize(){screenWidth=$(window).width();screenHeight=$(window).height();if(screenWidth-40>$("#dbr_rt1").width()){$(".scroll-bar.horizontal, .bt-scroller").hide()}else{$(".scroll-bar.horizontal, .bt-scroller").show()}
$(".max_height, .content").height(screenHeight-60+"px");$("body").find('#custom_css').html("<style>.scroll-bar.horizontal { max-width: "+(screenWidth-100)+"px; </style>");$("#tab_dbr_rt1").width(screenWidth-40+"px");$(".inner",theadClone).width($(".tableFloatingHeaderOriginal:eq(1) .row_header").width()+"px");$(".inner",theadClone).height($(".tableFloatingHeaderOriginal:eq(1) .row_header").height()+"px")
$(".tableFloatingHeaderOriginal:eq(1) .row_header").height($(".inner",theadClone).height()+"px");$(theadClone).height($(".tableFloatingHeaderOriginal:eq(1) .row_header").height()+"px");$("body").find(".tableFloatingHeaderOriginal:eq(1) .row_header th").each(function(index,el){$(".inner div:eq("+index+")",theadClone).width($(this).width()+'px')});$(".inner",theadClone).css("left",$("body").find(".overview").css("left"))}
$(window).resize(refreshSize);function refreshScrollbar(){$(".divTableWithFloatingHeader .divTableWithFloatingHeader").customScrollbar("resize",!0)}
$(".divTableWithFloatingHeader").on("change",refreshScrollbar);$(".findit").on('keyup',refreshScrollbar);refreshSize();setTimeout(function(){$(".divTableWithFloatingHeader .divTableWithFloatingHeader").customScrollbar({updateOnWindowResize:!0,vScroll:!0,hScroll:!0,onCustomScroll:function(event,scrollData){$(".inner",theadClone).css("left",$("body").find(".overview").css("left"))}})},300);$("body").on('click','#rightScroller',function(event){$(".divTableWithFloatingHeader .divTableWithFloatingHeader").customScrollbar("scrollByX",200);setTimeout(function(){$(".inner",theadClone).css("left",$("body").find(".overview").css("left"))},300)});$("body").on('click','#leftScroller',function(event){$(".divTableWithFloatingHeader .divTableWithFloatingHeader").customScrollbar("scrollByX",-200);setTimeout(function(){$(".inner",theadClone).css("left",$("body").find(".overview").css("left"))},300)});$("body").on('click','#topScroller',function(event){$(".divTableWithFloatingHeader .divTableWithFloatingHeader").customScrollbar("scrollByY",200)});$("body").on('click touch','.inline-edit',function(event){event.preventDefault();var target=$(event.target);if(target.is(".verify")){var editArea=$(this).find('.inline-edit-area');var orgElm=$(this);$(orgElm).html($(editArea).find("input").val());$(orgElm).removeClass('editing');return}
if($(this).hasClass('editing'))
return!1;$(this).addClass('editing');var oldText=$(this).text();$(this).html('<div class="inline-edit-area"><input type="text" value="'+oldText+'"><div class="verify_bg"><button class="verify">&#10003;</button></div></div>')})})